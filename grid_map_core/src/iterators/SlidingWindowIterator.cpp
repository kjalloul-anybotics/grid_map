...
const Matrix SlidingWindowIterator::getData() const
{
  ...
  switch (edgeHandling_) {
    ...
    case EdgeHandling::CROP:
      return data_.block(
        topLeftIndex(0), topLeftIndex(1), adjustedWindowSize(0), adjustedWindowSize(
          1));
    case EdgeHandling::EMPTY:
    case EdgeHandling::MEAN:
      const Matrix data = data_.block(
        topLeftIndex(0), topLeftIndex(1), adjustedWindowSize(
          0), adjustedWindowSize(1));
      ...
      returnData.block(
        topLeftIndexShift(0), topLeftIndexShift(1), adjustedWindowSize(
          0), adjustedWindowSize(1)) =
...
void SlidingWindowIterator::setup(const GridMap & gridMap)
{
  if (!gridMap.isDefaultStartIndex()) {
    throw std::runtime_error(
            "SlidingWindowIterator cannot be used with grid maps"
